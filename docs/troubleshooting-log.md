...
| 조치 | `aws_eks_access_entry.nodes`(EC2_LINUX) + nodegroup `depends_on`. destroy 전 `terraform-destroy-preflight.sh`(Access Entry 복구·ALB/TG 삭제·EIP/ENI·잔여 EC2 정리) + 실패 시 1회 재시도 |
| 재발 방지 | destroy CD 가 preflight 필수. 재apply 시 기존 Access Entry import |
| 계획 변경 | 없음 |
